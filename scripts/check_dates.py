...
anime_check = [
    ...
    ("Attack on Titan S2", 11741, 1429, 2),
    ("Attack on Titan S3", 22319, 1429, 3),
    ("Attack on Titan S4P1", 110277, 1429, 4),
    ("Death Note S1", 1535, 13916, 1),
    ("Jujutsu Kaisen S1", 51009, 95479, 1),
    ("Demon Slayer S1", 38000, 85937, 1),
    ("FMA Brotherhood S1", 5114, 31911, 1),
    ("Steins;Gate S1", 9253, 60574, 1),
    ("Cowboy Bebop S1", 1, 37854, 1),
...
for name, al_id, tmdb_id, season_num in anime_check:
    al_query = json.dumps({
        "query": f"query{{Media(id:{al_id}){{title{{romaji english}}startDate{{year month day}}endDate{{year month day}}episodes format}}}}"
    }).encode('utf-8')
    ...
    tmdb_eps = tmdb_data.get('episodes', [])
    if not tmdb_eps:
        print(f"No episodes for {name} season {season_num}")
        time.sleep(1)
        continue
    
    tmdb_first_date = tmdb_eps[0].get('air_date')
    tmdb_last_date = tmdb_eps[-1].get('air_date')
    tmdb_total = len(tmdb_eps)
    ...
    results.append({
        'name': name, 'al_start': al_start, 'al_end': al_end, 'al_eps': al_eps,
